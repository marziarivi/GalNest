...
RL_MeasurementSet* ms_open(const char* filename)
{
    ...
    {
        fprintf(stderr, "This code can read Measurement Sets with one spectral "
    ...
    {
        fprintf(stderr, "This code can read Measurement Sets with one target "
    ...
    if (p->num_pols != 1)
    {
        fprintf(stderr, "This code can only read Measurement Sets with a sigle polarization (I stokes)\n");
        fflush(stderr);
        ms_close(p);
        return 0;
    }
    ...
    if (p->ms->spectralWindow().nrow() > 0)
    {
        ...
        p->freq_start_hz = (p->msc->spectralWindow().chanFreq().get(0))(IPosition(1, 0));
...
/*
   Read a block of flags from the specified column of the main table of 
   the Measurement Set.
